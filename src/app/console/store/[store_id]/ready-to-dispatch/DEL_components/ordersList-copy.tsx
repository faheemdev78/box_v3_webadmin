...
import { DevBlock, Icon, Loader } from '@/components';
...
export const OrdersList: React.FC<OrdersListProps> = ({ limit = 50, page = 1, _id_store }) => {
  ...
  const [getReadyToDispatchQueue, { called, ...queue_restuls}] = useLazyQuery<any>(GET_ORDER_QUEUE);
  ...
  const columns = [
    ...
    { title: 'Delivery', dataIndex: ['delivery_slot'], key: 'delivery_slot', width: 160,
      render: (delivery_slot: any) => (<Space orientation="vertical" size={0}>
