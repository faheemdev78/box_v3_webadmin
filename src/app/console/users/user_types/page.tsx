...
function TypeForm({ onSuccess, onCancel, show, initialValues }: { onSuccess: () => void, onCancel: () => void, show: boolean, initialValues: UserRoleValues }) {
  ...
  const [addUserRole, add_details] = useMutation<any>(ADD_ROLE); // { data, loading, error }
  const [editUserRole, edit_details] = useMutation<any>(EDIT_ROLE); // { data, loading, error }
...
function UserTypes() {
  ...
  const [getRoles, { called, loading, data }] = useLazyQuery<any>(GET_LIST, { fetchPolicy: 'network-only' });
