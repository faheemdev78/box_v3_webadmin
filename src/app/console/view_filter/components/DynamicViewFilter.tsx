...
export const DynamicViewFilter: React.FC<DynamicViewFilterProps> = ({
  ...
  const { data: configData, loading: configLoading, error: entity_error } = useQuery<any>(GET_ENTITY_CONFIG, {
  ...
  // Fetch saved views
  const { data: viewsData, loading: viewsLoading, refetch: refetchViews, error: views_error } = useQuery<any>(GET_SAVED_VIEWS, {
  ...
  useEffect(() => {
    if (views_error) return;
    if (viewsData?.savedViews){
      // Set first view as active if none selected
      if (!activeViewId && viewsData?.savedViews?.edges?.length > 0) {
        setActiveViewId(viewsData.savedViews.edges[0]._id);
      }
    }
  }, [viewsData, views_error])
  ...
  // Fetch filtered data
  const { data: filteredDataResult, loading: dataLoading, refetch: refetchData, error: data_error } = useQuery<any>(GET_FILTERED_DATA, {
    variables: {
      entityType,
      viewId: activeViewId,
      limit: pageSize,
  ...
  let error = filteredDataResult?.filteredData?.error;
  if (!error && entity_error) error = entity_error;
  if (!error && views_error) error = views_error;
  if (error) message.error(error.message, 1.5)

  // Mutations
  const [createView] = useMutation<any>(CREATE_SAVED_VIEW, {
    onCompleted: () => {
  ...
  const [updateView] = useMutation<any>(UPDATE_SAVED_VIEW, {
    onCompleted: () => {
  ...
  const [deleteView] = useMutation<any>(DELETE_SAVED_VIEW, {
    onCompleted: () => {
  ...
  const [pinView] = useMutation<any>(PIN_SAVED_VIEW, {
    onCompleted: () => {
      refetchViews();
    },
