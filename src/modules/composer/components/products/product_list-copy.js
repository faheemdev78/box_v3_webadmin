...
function ProductList({ onProductsLoad, item: { data, schedule_start, schedule_end, values, sort_order, styles, status, name } }) {
    ...
    const [productsQuery, { data: __data, called, loading }] = useLazyQuery(LIST_DATA, { fetchPolicy: "network-only" } );
                    ...
                    {itemsArray.map((item, i) => {
                        ...
                        return (<Col span={8} key={i}>
                            ...
                            {item?.attributes?.length > 0 && <Space>
                                {item?.attributes?.map((o, ii) => (<div style={{ border: "1px solid #EDEFF3", backgroundColor: "#F5F6FB" }} key={ii}>{o.val}{o.title}</div>))}
                            </Space>}
