...
export const BasketSelector: React.FC<BasketSelectorProps> = ({
  ...
  const onSearch = useCallback(async (txt: string) => {
    ...
    const resutls = await get_getAvailableBaskets({
      variables: {
        _id_store: storeId, category, limit: 50,
    ...
    .then(r => checkApolloRequestErrors({ results: r, allowEmpty: true, parseReturn: (rr: any) => rr?.data?.getAvailableBaskets }))
    ...
    <Space orientation="vertical" size={16} style={{ width: '100%' }}>
      ...
      {/* <Alert
        description={<Space>
          ...
          {selectedBaskets.length < minRequired && (<Tag color="warning">{minRequired - selectedBaskets.length} more required</Tag>)}
          {selectedBaskets.length >= minRequired && (<Tag color="success" icon={<CheckCircleOutlined />}>Ready</Tag>)}
        </Space>}
